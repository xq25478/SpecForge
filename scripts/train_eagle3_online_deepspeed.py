...
"""
使用 flex-attention 会出现 OOM
    改成 sdpa 会更容易出现OOM
    改成 TP=2，DP=32  直接强改  会出现通信错误
    改成 cpu_offload=CPUOffload(offload_params=True),  # 参数放CPU 不会 OOM  训练速度会变慢 1 个小时->1.5 个小时

    SHARD_GRAD_OP 效果：  50 个小时
"""
